...
class DMCWrapper(gym.Wrapper):
    def __init__(
        ...
        * If only `from_pixels` is True, then the observation will be the image rendered
        ...
        if from_pixels:
            ...
            # Extra img observation
            camera_height = 64
            camera_width = 64

            camera_rgb_shape = (1, camera_height, camera_width) if channels_first else (camera_height, camera_width, 1)
            obs_space["camera_rgb"] = spaces.Box(low=0, high=255, shape=camera_rgb_shape, dtype=np.uint8)

            camera_depth_shape = (1, camera_height, camera_width) if channels_first else (camera_height, camera_width, 1)
            obs_space["camera_depth"] = spaces.Box(low=0, high=255, shape=camera_depth_shape, dtype=np.uint8)
    ...
    def _get_obs(self, time_step) -> Dict[str, np.ndarray]:
        ...
        if self._from_pixels:
            ...
            camera_rgb_shape = (1, camera_height, camera_width) if self._channels_first else (camera_height, camera_width, 1)
            random_rgb_img = np.random.randint(0, 256, size=camera_rgb_shape, dtype=np.uint8)
            obs["camera_rgb"] = random_rgb_img

            camera_depth_shape = (1, camera_height, camera_width) if self._channels_first else (camera_height, camera_width, 1)
            random_depth_img = np.random.randint(0, 256, size=camera_depth_shape, dtype=np.uint8)
            obs["camera_depth"] = random_depth_img
