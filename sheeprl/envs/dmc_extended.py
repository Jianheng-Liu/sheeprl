...
class DMCWrapper(gym.Wrapper):
    def __init__(
        ...
        * If only `from_pixels` is True, then the observation will be the image rendered
        ...
        if from_pixels:
            ...
            obs_space["random_img"] = spaces.Box(low=0, high=255, shape=shape, dtype=np.uint8)
            
        if from_vectors:
            obs_space["state"] = _spec_to_box(self.env.observation_spec().values(), np.float64)

            # Extra vector observation
            obs_space["random_values"] = spaces.Box(low=0, high=1, shape=(10,), dtype=np.float32)

        if from_pixels and from_vectors:
            # Generate new observation
            combined_shape = 1
            obs_space["combined_values"] = spaces.Box(low=float('-inf'), high=float('inf'), shape=(combined_shape,), dtype=np.float32)
    ...
    def _get_obs(self, time_step) -> Dict[str, np.ndarray]:
        ...
        if self._from_pixels:
            ...
            # random image
            random_img = np.random.randint(0, 256, size=rgb_obs.shape, dtype=np.uint8)
            obs["random_img"] = random_img
        ...
        if self._from_vectors:
            ...
            obs["random_values"] = random_values

        # Extra combined values
        if self._from_pixels and self._from_vectors:
            image_first_value = rgb_obs[0, 0, 0] if self._channels_first else rgb_obs[0, 0, 0]
            vector_first_value = vec_obs[0]
            combined_values = np.array([image_first_value + vector_first_value], dtype=np.float32)
            obs["combined_values"] = combined_values
